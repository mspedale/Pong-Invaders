...
public class script_InvaderFleet2 : MonoBehaviour {
	...
	void Update () {
		...
		if (restTime <= 0)
		{
			...
			if(restTime+1 > 0) // if restTime reached 0 on THIS frame
			{
				...
				if (wallAhead == true) {
					// StepY
					destination.y = currentPosition.y + stepYsize;
			...
			else // Destination reached
			{
				...
				// Y movement
				if (currentPosition.y < destination.y)  // Not yet reached destination
				{
					newVelocity.y = stepSpeed;
				}
				else // Destination reached
				{
					newVelocity.y = 0f;
